...
from ipywidgets import Tab, Output, VBox, Layout

# --- FONT CHOICES ---
GLOBAL_FONT = "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif"
...
for _, row in persona_cards_df.iterrows():
    ...
    with out:
        ...
        <style>
            ...
            .persona-body .field-label {{
                font-size: 15px;
                font-weight: 600;
                margin-top: 12px;
                margin-bottom: 4px;
            }}
            .persona-body .field-value {{
                font-size: 15px;
                font-weight: 400;
            }}
        </style>
        <div class="persona-container">
            <div class="persona-header">
                <div class="persona-header-row">
                    <div class="persona-header-item">
                        <div class="emoji">👤</div>
                        <b>Segment Size:</b> {row.get('n_customers', 'N/A')}
                    </div>
                    <div class="persona-header-item">
                        <div class="emoji">⭐</div>
                        <b>Avg Rating:</b> {row.get('avg_rating', 'N/A')}
                    </div>
                    <div class="persona-header-item">
                        <div class="emoji">😐</div>
                        <b>Avg Sentiment:</b> {row.get('avg_sentiment', 'N/A')}
                    </div>
                    <div class="persona-header-item">
                        <div class="emoji">💲</div>
                        <b>Avg Price:</b> ${row.get('mean_price', 'N/A')}
                    </div>
                </div>
            </div>
            <div class="persona-body">
                <h3>Persona {cid}: {row['persona_name']}</h3>

                <div class="field-label">Core Brand Orientation:</div>
                <div class="field-value">{format_list(row['core_brands'])}</div>

                <div class="field-label">Favorite Products:</div>
                <div class="field-value">{format_list(row['favorite_products'])}</div>

                <div class="field-label">Skin Types:</div>
                <div class="field-value">{format_list(row['skin_type_cluster'])}</div>

                <div class="field-label">Signature Brands (Lift-Based):</div>
                <div class="field-value">{format_list(row['signature_brands'])}</div>

                <div class="field-label">Signature Products (Lift-Based):</div>
                <div class="field-value">{format_list(row['signature_products'])}</div>

                <div class="field-label">Pain Points:</div>
                <div class="field-value">{format_list(row['pain_points'])}</div>
            </div>
        </div>
        """
        display(HTML(card_html))

    tab_contents.append(VBox([out], layout=Layout(margin="0px")))
